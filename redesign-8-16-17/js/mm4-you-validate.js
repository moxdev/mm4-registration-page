(function() {
	...
	function validateForm(form, msgBox, e) {
		...
		var checkboxVerify = document.querySelectorAll('input[type=checkbox]');
		var empty = [].filter.call( checkboxVerify, function( el ) {
		   return !el.checked
		});

		if (checkboxVerify.length == empty.length) {
		    errors.push([checkboxVerify.name, '<span>The <strong><em>' + checkboxes[0].getAttribute('data-error-label') + '</em></strong> 1 checkbox is required.</span>']);
		}
